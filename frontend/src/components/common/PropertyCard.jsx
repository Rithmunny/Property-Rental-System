  ...
  return (
    <Link to={`/listings/${property.id}`} className="group block">
      <div className="relative aspect-square overflow-hidden rounded-xl">
        ...
        >
          <Heart
            className={`h-6 w-6 drop-shadow ${saved ? 'fill-primary text-primary' : 'fill-black/30 text-white'}`}
          />
        </button>

        {!property.available && (
          <span className="absolute left-3 top-3 rounded-full bg-card px-3 py-1 text-xs font-semibold text-foreground shadow">
            Rented
          </span>
        )}
      </div>
      ...
      <div className="mt-2">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          <span className="font-semibold text-foreground">${property.price}</span> month
        </p>
      </div>
    </Link>
  )
