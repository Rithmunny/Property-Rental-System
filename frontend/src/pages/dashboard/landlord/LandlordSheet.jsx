  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <div className="print-sheet mt-6 overflow-hidden rounded-2xl border border-border bg-card">
          <div className="hidden border-b border-border px-5 py-4 print:block">
            <p className="text-lg font-bold text-foreground">PRS landlord sheet</p>
            <p className="text-sm text-muted-foreground">
              Prepared {new Date().toLocaleDateString()} · {user?.name || 'Landlord'}
            </p>
          </div>

          <div className="grid grid-cols-2 gap-3 border-b border-border px-4 py-4 sm:grid-cols-4">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full text-left text-sm">
              ...
              <tbody>
                {rows.map((row) => (
                  <tr key={row.tenant.id} className="border-t border-border">
                    <td className="px-4 py-3">
                      <p className="font-medium text-foreground">{row.tenant.name}</p>
                      <p className="text-xs text-muted-foreground">{row.tenant.telegram}</p>
                    </td>
                    <td className="px-4 py-3">
                      <p className="text-foreground">{row.property?.title || '—'}</p>
                      <p className="text-xs text-muted-foreground">
                        {[row.property?.neighbourhood, row.property?.city].filter(Boolean).join(', ')}
                      </p>
                    </td>
                    <td className="px-4 py-3 font-semibold text-foreground">{money(row.tenant.rent)}</td>
                    ...
                    <td className="px-4 py-3 text-muted-foreground">{row.reminder?.dueDate || '—'}</td>
